...
def ingest_sensor_data():
    ...
    print(data)
    required_fields = ["temperature", "pressure", "vibration", "power", "status", "uptime", "efficiency", "operator"]
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO machine_status (timestamp, temperature, pressure, vibration, power, status, uptime, efficiency, operator)
        VALUES (datetime('now'), ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        data["temperature"],
        data["pressure"],
